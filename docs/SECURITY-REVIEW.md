...
- **Privacy readiness:** legal operator identity/address/contact, actual database region, email provider, processing agreements, international transfers, and retention/backup schedules remain unverified. The owner confirmed only a personal, non-commercial beta in Germany. Legal pages remain visibly incomplete until required details are configured.
...
## Operator Setup
Set `LEGAL_OPERATOR_NAME`, `LEGAL_OPERATOR_ADDRESS`, and `LEGAL_CONTACT_EMAIL` in the deployment environment and rebuild. These values are intentionally public website disclosures, not credentials. Keep private secrets out of source and chat. Configuration alone does not establish legal compliance; review all unverified policy statements and any additional German disclosure obligations before broader release.
