...
- **Privacy readiness:** the operator supplied a public name, email, and full Dortmund postal address on 2026-09-12. These contact details are published as operator-supplied, not independently verified. Actual database region, email provider, processing agreements, international transfers, and retention/backup schedules remain unconfirmed. Removing the incomplete-contact warning does not establish legal compliance.
...
## Operator Setup
Public operator defaults include the supplied address Emil-Figge-Str. 21, 44227 Dortmund, Germany. They can be overridden with `LEGAL_OPERATOR_NAME`, `LEGAL_OPERATOR_ADDRESS`, and `LEGAL_CONTACT_EMAIL` in the deployment environment. A custom address requires `LEGAL_OPERATOR_ADDRESS_CONFIRMED=true` after verification, then a rebuild. Do not confirm a town/postcode/country alone. Setting the flag to `false` also marks the default address unconfirmed. These values are intentionally public website disclosures, not credentials. Keep private secrets out of source and chat. Configuration alone does not establish legal compliance; review all unverified policy statements and any additional German disclosure obligations before broader release.

On 2026-09-12 the operator reported completing Groq settings after receiving ZDR guidance. No live Groq settings have been changed or independently verified here. Verify the effective configuration before allowing personal recordings; server credentials, migration deployment, cleanup, account isolation, and live benchmarking remain separate launch gates.
