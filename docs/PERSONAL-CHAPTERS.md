...
Status: creation is enabled in Production, but live generation is currently blocked by Groq rejecting server access (provider 401/403). Signed-in library access and private job reservation work after repairing the Supabase server credential. The signed-in Groq organization shows the Free plan and Global ZDR enabled. No generated chapter or transcription has passed a live test. This is a limited personal beta, not unlimited free AI or certified language assessment.
...
Two subsequent synthetic generation attempts reached the provider stage but failed without saving a chapter. Safe error classification deployed in `5a9141b` identifies the remaining failure as provider access rejection (401/403), not malformed learner text. Raw provider responses, keys, and input text are not included in these errors. Both daily generation attempts count; quotas were not reset or relaxed. A valid Groq key with access to the configured models must be supplied privately before further verification. Successful cleanup execution, live transcription, and generated German quality remain unverified. No billing plan, provider privacy setting, application quota, consent check, or ownership control was changed.
...
- **Vercel:** the production site, master-branch automatic deployments, `app/` root, Production scopes for the three server secrets, and active cleanup schedule are verified through CLI. Creation is enabled at the operator's request despite the remaining verification gaps. Groq access and successful authenticated cleanup still need repair/verification; unset the flag or set it to `false` and redeploy to disable new creation.
...
- **Groq:** the signed-in organization's dashboard confirms Free ($0), Global ZDR enabled, Inference ZDR enabled, and Batch plus fine-tuning/LoRA storage off. Those settings were inspected, not changed. The deployed key is rejected for provider access, so its association with this organization and configured-model permissions are not established. Usage can lag by 15 minutes. No live generation or transcription benchmark has passed; repair the key, run a synthetic smoke test, review German quality, and check consented device audio before relying on generated lessons or submitting personal recordings.
