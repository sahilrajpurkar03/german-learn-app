...
Status: the operator requested production activation on 2026-09-13 after reviewing the outstanding checks. `PERSONAL_CHAPTERS_ENABLED=true` is configured in Production and a redeployment is Ready. Signed-in generation/transcription, Free-plan configuration, and German quality remain unverified. This is a limited personal beta, not unlimited free AI or certified language assessment.

## Remaining Setup (2026-09-13)

### Latest CLI Verification

Vercel CLI confirms the three server secrets are present in Production and the daily 03:00 UTC cleanup cron is enabled. The production creation flag was set to the exact value `true`; redeployment `dpl_H5GEnyVVV6f1c56V6CxZfTsfT9bj` reached Ready and was aliased to the live site. The login page returned 200, and unauthenticated chapter-library and cleanup requests returned 401. Secret validity, successful authenticated cleanup, and a signed-in generation request are not verified. No billing plan, provider privacy setting, application quota, consent check, or ownership control was changed.
...
- **Vercel:** the production site, master-branch automatic deployments, `app/` root, Production scopes for the three server secrets, and active cleanup schedule are verified through CLI. Creation is enabled at the operator's request despite the remaining verification gaps. Secret validity and a successful authenticated cleanup execution remain unverified; unset the flag or set it to `false` and redeploy to disable new creation.
- **Supabase:** CLI access, migration deployment, database permissions, and the affected-schema restore rehearsal are verified. The deployed service-role secret and two-account database/Storage API isolation remain unverified. Also verify production auth redirect URLs, real signup/recovery email delivery, full-service disaster recovery, and administrator MFA.
- **Groq:** the settings URL currently leads to the sign-in home, not verified data controls. The operator reported completing retention settings, but effective ZDR, Free-plan/model availability, quotas, and a valid deployed key still need confirmation. No live generation or transcription benchmark has passed. Run a synthetic smoke test, review German quality, and check consented device audio before relying on generated lessons or submitting personal recordings.
