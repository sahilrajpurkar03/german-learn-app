...
The existing `sprechen-studio-v1:<userId>` record has a default-empty `recall` field, so old saved state is retained. It stores identifiers, aggregate counts, last-response state, and scheduling dates, not typed replies or recordings. `attempts` counts completed response observations, not every keystroke or retry. An interrupted daily round keeps completed observations but does not preserve the exact unfinished queue.
