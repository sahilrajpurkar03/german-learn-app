...
import { scramble } from "./random.ts";
import { tileDistractors } from "./distractors.ts";
...
export function conversationStep(mission: Mission, index: number, bank: Item[]): Step {
  const turn = mission.turns[index];
  ...
  if (turn.kind === "build") {
    ...
    return { ...base, type: "build", tiles: scramble([...words, ...tileDistractors(words, bank, id)], id) };
  }
  return { ...base, type: "respond" };
}

export function conversationLesson(mission: Mission, unitId: string, bank: Item[] = []): Lesson {
  const steps = mission.turns.map((_, index) => conversationStep(mission, index, bank));
  return {
